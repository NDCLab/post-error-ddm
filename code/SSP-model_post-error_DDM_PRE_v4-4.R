...
## Run on NIH Flanker children's pre-assessment (session 1) data. 
## See multiSubImport_cleanAccRT for cleaning steps. 
## This script was run on the HPC
## 10,000 trials
## Creates one csv file with both post-error and post-correct
## Appends each subject to the output csv when they're complete, instead of printing all of them at the end
...
fitFunctionSSP <- function(parms, nTrials, cutPoints, humanProps, HumanTrialCounts){
  ...
  Rcpp::sourceCpp("/home/data/NDClab/analyses/post-error-ddm/code/simSSP_model_GB_noScale.cpp")
  #Rcpp::sourceCpp("~/OneDrive - Florida International University/Research/DDM project/masters-project/ddm-materials/simSSP_model_GB_noScale.cpp")
  ...
  #OAS- make sure that the "simSSP_model_GBnoScale" bit is called whatever it calls itself in the actual .cpp file
...
#library("DEoptim", lib.loc="C:/Users/ostibolt/Desktop/DDM/R_proj_DDM/packages")
#library("Rcpp", lib.loc="C:/Users/ostibolt/Desktop/DDM/R_proj_DDM/packages")
library("DEoptim")
library("Rcpp")

#switch to the desired working directory
setwd("/home/data/NDClab/analyses/post-error-ddm/code/processing-ddm/2022-08-30_run")
#setwd("~/OneDrive - Florida International University/Research/DDM project/masters-project/ddm-materials/")
...
#names files
# HumanDataName <-"quickDat_clean_PRE_flanker.csv"
myargs = commandArgs(trailingOnly=TRUE)
HumanDataName <- myargs[1]
# FitOutputName <- "5trialFits_For_quickDat"
FitOutputName <-"10000trialFits_for_v6-0_clean_PRE_flanker_2022-08-29_10-PM_rundate-2022-08-30"

#get the desired humann data to fit the ssp model to
importDat = read.csv(HumanDataName, header = TRUE) #data has header

#convert rt values from secs to ms (ms to secs?) to be consistent with rest of script
...
for(c in 1:2){
  ...
  #for(s in 2:4){
    ...
    #data = subData; #need this to stay consistent with rest of code since commenting out conditions if not runing post-err/corr condition
