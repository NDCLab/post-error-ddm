...
fitFunctionSSP <- function(parms, nTrials, cutPoints, humanProps, HumanTrialCounts){
  ...
  Rcpp::sourceCpp("~/OneDrive - Florida International University/Research/DDM project/behavioral-data_ddm-scripts_ddm-outputs/simSSP_model_GB_noScale.cpp")
...
setwd("~/OneDrive - Florida International University/Research/DDM project/behavioral-data_ddm-scripts_ddm-outputs/ABMT_1_files_only/")
...
#names files
HumanDataName <-"OAS_modify_multiSub_clinicFlanker_behavior_clean2021_02_26_03_PM.csv"
FitOutputName <-"OAS-2_50trialFits_5sdBounds_iterMax200_stepTol20_For_multiSub_clinicFlanker_behavior_clean2021_12_30_03_PM_Subs6to15_"
...
# OAS- Make a new column that will describe post-error (100) and post-correct (111)
condition <- c("HOLD", ifelse(importDat$accuracy <1, 100, 111))
importDat$condition <- condition[1:(length(condition)-1)]
...
for(c in 1:2){
  ...
  fitOutput <- matrix(0, nrow = length(subList), ncol = 8)
  colnames(fitOutput) <- c("subject", "a", "ter", "p", "rd", "sda","fitStat","iterNum")
  ...
  #for(s in 6:15){
    ...
    #data = subData; #need this to stay consistent with rest of code since commenting out conditions
  ...
  ## commenting out part about condition when saving  bc only doing one condition for now
  ...
  write.csv(fitOutput, paste0(FitOutputName,".csv"), row.names=FALSE, na="", quote = F) # generated parms
